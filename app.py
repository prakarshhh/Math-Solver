...
with st.container():
    ...
    # Button for finding the answer
    if st.button("Find My Answer"):
        if question:
            with st.spinner("Generating response..."):
                st.session_state.messages.append({"role": "user", "content": question})
                st.markdown(f'<div class="stChatMessage user">{question}</div>', unsafe_allow_html=True)
                response = assistant_agent({"input": question})
                st.markdown(f'<div class="stChatMessage assistant">{response["output"]}</div>', unsafe_allow_html=True)
